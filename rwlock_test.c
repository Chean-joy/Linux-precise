...
void * lock_reader(void * params)
{
    pthread_rwlock_rdlock(&rwlock);

    printf("now is %s ,shard_date is %d\n",((char*)params),shard_data);

    pthread_rwlock_unlock(&rwlock);
}


void * lock_writer(void * params)
{
    pthread_rwlock_wrlock(&rwlock);
    int tmp  = shard_data +1;
    sleep(1);
    shard_data = tmp;

    printf("now is %s,shared_date is %d\n",((char*)params),shard_data);
